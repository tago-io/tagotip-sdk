...
use tagotip_codec::types::{HeadlessFrame, Method, Operator, PushBody, StructuredBody, Value, Variable};
use tagotip_secure::{
  CipherSuite, bytes_to_hex, derive_auth_hash, derive_device_hash, derive_key, hex_to_bytes, is_envelope,
  open_envelope, parse_envelope_header, seal_uplink,
};
...
const EXPECTED_AAD: [u8; 21] = [
  0x00, 0x00, 0x00, 0x00, 0x2a, 0x4d, 0xee, 0xdd, 0x7b, 0xab, 0x88, 0x17, 0xec, 0xab, 0x77, 0x88, 0xd2, 0x2e, 0xb7,
  0x37, 0x2f,
];

const EXPECTED_CIPHERTEXT: [u8; 20] = [
  0xc8, 0xc5, 0xaa, 0x56, 0xd7, 0x55, 0x58, 0x2b, 0xac, 0xea, 0x13, 0xbb, 0x57, 0x24, 0x93, 0xbb, 0x8c, 0xb1, 0x08,
  0x03,
];
...
fn test_nonce_construction() {
  let nonce =
    tagotip_secure::nonce::construct_nonce(CipherSuite::Aes128Ccm, EXPECTED_FLAGS, &EXPECTED_DEVICE_HASH, COUNTER);
...
fn test_seal_open_with_derived_key() {
  ...
  let envelope = seal_uplink(Method::Push, &frame, 1, auth_hash, key16, CipherSuite::Aes128Ccm).unwrap();
