...
use tagotip_secure::{
  CipherSuite, Flags, derive_auth_hash, is_envelope, open_envelope, parse_envelope_header,
  seal_raw, seal_uplink, EnvelopeMethod,
};
...
fn test_wrong_key() {
  ...
  let envelope =
    seal_uplink(Method::Ping, &frame, 1, auth_hash, &KEY_16, CipherSuite::Aes128Ccm).unwrap();
