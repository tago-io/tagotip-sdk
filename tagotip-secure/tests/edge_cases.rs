...
use tagotip_secure::{
  CipherSuite, EnvelopeMethod, Flags, derive_auth_hash, is_envelope, open_envelope, parse_envelope_header, seal_raw,
  seal_uplink,
};
...
fn test_tampered_header() {
  ...
  let mut envelope = seal_uplink(Method::Ping, &frame, 1, auth_hash, &KEY_16, CipherSuite::Aes128Ccm).unwrap();
...
fn test_reserved_flags_value_encode() {
  ...
  assert_eq!(result.unwrap_err().kind, CryptoErrorKind::ReservedFlagsValue);
}
...
fn test_unsupported_version_decode() {
  ...
  assert_eq!(result.unwrap_err().kind, CryptoErrorKind::UnsupportedVersion);
}
...
fn test_flags_round_trip() {
  ...
  for &(suite, _) in &suites {
    for &method in &methods {
      ...
      match result {
        Ok(byte) => {
          let (decoded_suite, decoded_version, decoded_method) = Flags::decode(byte).unwrap();
...
fn test_cipher_suite_from_id() {
  ...
  assert_eq!(CipherSuite::from_id(4).unwrap(), CipherSuite::ChaCha20Poly1305);
...
fn test_envelope_method_conversions() {
  assert_eq!(EnvelopeMethod::from(tagotip_codec::Method::Push), EnvelopeMethod::Push);
  assert_eq!(EnvelopeMethod::from(tagotip_codec::Method::Pull), EnvelopeMethod::Pull);
  assert_eq!(EnvelopeMethod::from(tagotip_codec::Method::Ping), EnvelopeMethod::Ping);
