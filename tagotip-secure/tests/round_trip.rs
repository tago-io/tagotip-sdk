...
use tagotip_codec::types::{
  AckDetail, AckFrame, AckStatus, HeadlessFrame, Method, Operator, PullBody, PushBody, StructuredBody, Value, Variable,
};
use tagotip_secure::{
  CipherSuite, EnvelopeMethod, derive_auth_hash, derive_device_hash, open_envelope, seal_downlink, seal_uplink,
};
...
#[cfg(any(feature = "aes-256-ccm", feature = "aes-256-gcm", feature = "chacha20-poly1305"))]
const KEY_32: [u8; 32] = [
  0xfe, 0x09, 0xda, 0x81, 0xbc, 0x44, 0x00, 0xee, 0x12, 0xab, 0x56, 0xcd, 0x78, 0xef, 0x90, 0x12, 0x01, 0x02, 0x03,
...
fn test_chacha20_poly1305_ack() {
  ...
  let envelope = seal_downlink(&ack, 1, auth_hash, device_hash, &KEY_32, CipherSuite::ChaCha20Poly1305).unwrap();
