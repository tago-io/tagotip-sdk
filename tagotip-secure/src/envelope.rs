...
pub fn seal_uplink(
  ...
  seal_raw(inner_frame, envelope_method, counter, auth_hash, device_hash, encryption_key, cipher_suite)
}
...
pub fn seal_downlink(
  ...
  seal_raw(inner_frame, EnvelopeMethod::Ack, counter, auth_hash, device_hash, encryption_key, cipher_suite)
}
...
mod tests {
  ...
  fn test_seal_open_uplink_push() {
    ...
    let key: [u8; 16] = [0xfe, 0x09, 0xda, 0x81, 0xbc, 0x44, 0x00, 0xee, 0x12, 0xab, 0x56, 0xcd, 0x78, 0xef, 0x90, 0x12];
    ...
    let envelope = seal_uplink(Method::Ping, &frame, 42, auth_hash, &key, CipherSuite::Aes128Ccm).unwrap();
  ...
  fn test_seal_open_downlink_ack() {
    ...
    let envelope =
      seal_downlink(&ack, 1, auth_hash, device_hash, &key, CipherSuite::Aes128Ccm).unwrap();
