use crate::consts::{
  AES_128_KEY_SIZE, AES_256_KEY_SIZE, AUTH_HASH_SIZE, CCM_NONCE_SIZE, CCM_TAG_SIZE, COUNTER_SIZE, DEVICE_HASH_SIZE,
  FLAGS_CIPHER_MASK, FLAGS_CIPHER_SHIFT, FLAGS_METHOD_MASK, FLAGS_SIZE, FLAGS_VERSION_MASK, FLAGS_VERSION_SHIFT,
  GCM_NONCE_SIZE, GCM_TAG_SIZE, HEADER_SIZE, RESERVED_FLAGS_VALUE,
};
...
impl Flags {
  ...
  pub fn encode(cipher: CipherSuite, version: u8, method: EnvelopeMethod) -> Result<u8, CryptoError> {
    if version > 3 {
      return Err(CryptoError::unsupported_version());
    }
    let byte = (cipher.id() << FLAGS_CIPHER_SHIFT) | (version << FLAGS_VERSION_SHIFT) | method.id();
