...
fn encrypt_aes128_ccm(key: &[u8], nonce: &[u8], aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
  ...
  let cipher = Aes128Ccm::new_from_slice(key).map_err(|_| CryptoError::invalid_key_size())?;
...
fn encrypt_aes128_gcm(key: &[u8], nonce: &[u8], aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
  ...
  let cipher = Aes128Gcm::new_from_slice(key).map_err(|_| CryptoError::invalid_key_size())?;
...
fn encrypt_aes256_ccm(key: &[u8], nonce: &[u8], aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
  ...
  let cipher = Aes256Ccm::new_from_slice(key).map_err(|_| CryptoError::invalid_key_size())?;
  let nonce = ccm::aead::generic_array::GenericArray::from_slice(nonce);
  let payload = Payload { msg: plaintext, aad };
  cipher
    .encrypt(nonce, payload)
...
fn encrypt_aes256_gcm(key: &[u8], nonce: &[u8], aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
  ...
  let cipher = Aes256Gcm::new_from_slice(key).map_err(|_| CryptoError::invalid_key_size())?;
  let nonce = aes_gcm::aead::generic_array::GenericArray::from_slice(nonce);
  let payload = Payload { msg: plaintext, aad };
  cipher
    .encrypt(nonce, payload)
...
fn encrypt_chacha20_poly1305(key: &[u8], nonce: &[u8], aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
  use chacha20poly1305::ChaCha20Poly1305;
  use chacha20poly1305::aead::{Aead, KeyInit, Payload};

  let cipher = ChaCha20Poly1305::new_from_slice(key).map_err(|_| CryptoError::invalid_key_size())?;
