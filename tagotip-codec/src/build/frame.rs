...
use crate::fmt;
use crate::types::{
  AckDetail, AckFrame, AckStatus, HeadlessFrame, MetaPair, MetaRange, Method, Operator, PassthroughEncoding, PullBody,
  PushBody, UplinkFrame, Value, Variable,
};
...
impl<'buf> FrameWriter<'buf> {
  ...
  fn write_u32(&mut self, value: u32) -> Result<(), BuildError> {
    let n = fmt::format_u32(value, &mut self.buf[self.pos..]).ok_or_else(BuildError::buffer_too_small)?;
    self.pos += n;
    Ok(())
  }
...
/// Returns the number of bytes written.
pub fn build_headless(method: Method, frame: &HeadlessFrame<'_>, buf: &mut [u8]) -> Result<usize, BuildError> {
  let mut w = FrameWriter::new(buf);
...
pub fn build_variable(var: &Variable<'_>, meta_pool: &[MetaPair<'_>], buf: &mut [u8]) -> Result<usize, BuildError> {
  let mut w = FrameWriter::new(buf);
