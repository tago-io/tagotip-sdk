...
pub fn parse_push_body<'a>(body: &'a str, base_pos: usize) -> Result<PushBody<'a>, ParseError> {
  ...
  let bracket_pos =
    find_unescaped_byte(bytes, b'[').ok_or_else(|| ParseError::new(ParseErrorKind::InvalidVariableBlock, base_pos))?;
  ...
  let end_bracket = find_closing_bracket(bytes, bracket_pos + 1)
    .ok_or_else(|| ParseError::new(ParseErrorKind::InvalidVariableBlock, base_pos + bracket_pos))?;
...
pub fn parse_pull_body<'a>(body: &'a str, base_pos: usize) -> Result<PullBody<'a>, ParseError> {
  ...
  if inner.is_empty() {
    return Err(ParseError::new(ParseErrorKind::InvalidVariableBlock, base_pos));
  }
  ...
  loop {
    ...
    if at_end || is_semi {
      ...
      if !name.is_empty() {
        ...
        variables
          .push(name)
          .map_err(|_| ParseError::new(ParseErrorKind::TooManyItems, base_pos + 1 + start))?;
      }
      if at_end {
        break;
      }
  ...
  while pos < bytes.len() {
    match bytes[pos] {
      ...
      b'{' => {
        ...
        pos += 1;
        let start = pos;
        let end = find_unescaped_byte(&bytes[pos..], b'}')
