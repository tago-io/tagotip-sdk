...
use crate::types::{AckFrame, HeadlessFrame, MetadataBlock, Method, PullBody, PushBody, UplinkFrame};
...
pub fn parse_headless(method: Method, input: &str) -> Result<HeadlessFrame<'_>, ParseError> {
  match method {
    Method::Push => {
      ...
      let (serial_str, body_str) =
        split_first_pipe(input).ok_or_else(|| ParseError::new(ParseErrorKind::MissingBody, 0))?;
