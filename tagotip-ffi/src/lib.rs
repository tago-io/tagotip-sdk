...
use tagotip_codec::types::{
  AckDetail, AckFrame, AckStatus, ErrorCode, MAX_TOTAL_META, Method, Operator, PassthroughEncoding, PushBody,
  UplinkFrame, Value,
};
...
pub unsafe extern "C" fn tagotip_build_uplink(
  ...
  let seq = if frame.has_seq != 0 { Some(frame.seq) } else { None };
...
/// Returns 0 on success, negative error code on failure.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn tagotip_parse_ack(input_ptr: *const u8, input_len: usize, out: *mut TagotipAckFrame) -> i32 {
  let input = unsafe {
    let bytes = slice::from_raw_parts(input_ptr, input_len);
    match str::from_utf8(bytes) {
      Ok(s) => s,
...
/// Returns bytes written on success, negative error code on failure.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn tagotip_build_ack(frame: *const TagotipAckFrame, buf_ptr: *mut u8, buf_len: usize) -> i32 {
  let frame = unsafe { &*frame };
  let buf = unsafe { slice::from_raw_parts_mut(buf_ptr, buf_len) };
