...
fn ffi_parse_uplink_simple_push() {
  ...
  assert!(matches!(frame.push_body_tag, TagotipPushBodyTag::Structured));
  assert_eq!(frame.variables_len, 2);
  assert_eq!(unsafe { str_from_tagotip(&frame.variables[0].name) }, "temperature");
  assert_eq!(unsafe { str_from_tagotip(&frame.variables[1].name) }, "humidity");
}
...
fn ffi_parse_uplink_typed_values() {
  ...
  // Number
  assert!(matches!(frame.variables[0].operator, TagotipOperator::Number));
  assert!(matches!(frame.variables[0].value.tag, TagotipValueTag::Number));
  assert_eq!(unsafe { str_from_tagotip(&frame.variables[0].value.str_val) }, "32.5");

  // String
  assert!(matches!(frame.variables[1].operator, TagotipOperator::String));
  assert!(matches!(frame.variables[1].value.tag, TagotipValueTag::String));
  assert_eq!(unsafe { str_from_tagotip(&frame.variables[1].value.str_val) }, "hello");

  // Boolean
  assert!(matches!(frame.variables[2].operator, TagotipOperator::Boolean));
  assert!(matches!(frame.variables[2].value.tag, TagotipValueTag::Boolean));
...
fn ffi_parse_uplink_location() {
  ...
  let (rc, frame) = unsafe { ffi_parse_uplink_helper(&input) };
  assert_eq!(rc, TAGOTIP_OK);
  assert!(matches!(frame.variables[0].value.tag, TagotipValueTag::Location));
  assert_eq!(unsafe { str_from_tagotip(&frame.variables[0].value.lat) }, "39.74");
  assert_eq!(unsafe { str_from_tagotip(&frame.variables[0].value.lng) }, "-104.99");
  assert_eq!(unsafe { str_from_tagotip(&frame.variables[0].value.alt) }, "305");
}
...
fn ffi_parse_uplink_passthrough() {
  ...
  let (rc, frame) = unsafe { ffi_parse_uplink_helper(&input) };
  assert_eq!(rc, TAGOTIP_OK);
  assert!(matches!(frame.push_body_tag, TagotipPushBodyTag::Passthrough));
  assert!(matches!(frame.passthrough.encoding, TagotipPassthroughEncoding::Hex));
  assert_eq!(unsafe { str_from_tagotip(&frame.passthrough.data) }, "DEADBEEF");
}
...
fn ffi_parse_uplink_pull() {
  ...
  assert_eq!(unsafe { str_from_tagotip(&frame.pull_variables[0]) }, "temperature");
  assert_eq!(unsafe { str_from_tagotip(&frame.pull_variables[1]) }, "humidity");
}
...
fn ffi_parse_ack_err() {
  ...
  assert!(matches!(frame.detail.error_code, TagotipErrorCode::InvalidToken));
}
