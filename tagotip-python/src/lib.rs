...
use tagotip_codec::types::{
  AckDetail, AckStatus, ErrorCode, Method, Operator, PassthroughEncoding, PushBody, StructuredBody, Value,
};
...
fn structured_body_to_dict<'py>(py: Python<'py>, sb: &StructuredBody<'_>) -> PyResult<Bound<'py, PyDict>> {
...
fn seal_uplink_native(
  ...
  let envelope_method = tagotip_secure::EnvelopeMethod::from_id(method).map_err(crypto_error_to_py)?;
...
fn open_envelope_native(py: Python<'_>, envelope: &[u8], key: &[u8]) -> PyResult<Py<PyDict>> {
  let (header, method, plaintext) = tagotip_secure::open_envelope(envelope, key).map_err(crypto_error_to_py)?;

  let dict = PyDict::new(py);
  dict.set_item("flags", header.flags)?;
  dict.set_item("counter", header.counter)?;
  dict.set_item("auth_hash", pyo3::types::PyBytes::new(py, &header.auth_hash))?;
  dict.set_item("device_hash", pyo3::types::PyBytes::new(py, &header.device_hash))?;
...
fn hex_to_bytes_native(py: Python<'_>, hex: &str) -> PyResult<Py<pyo3::types::PyBytes>> {
  let bytes = tagotip_secure::hex_to_bytes(hex).ok_or_else(|| PyValueError::new_err("invalid hex string"))?;
